...
class Order
{
    ...
    public function getTotalWt()
    {
        ...
        foreach ($this->getOrderDetails() as $product) {
            $coeff = ($product->getProductTva() / 100) + 1;
            $totalTTC += ($product->getProductPrice() * $coeff) * $product->getProductQuantity(); // ← += manquant
        }
        return $totalTTC + $this->getCarrierPrice();
    }
    public function getTotalTva()
    {
        $totalTva = 0; // ← initialisation à 0

        foreach ($this->getOrderDetails() as $product) {
            $coeff = $product->getProductTva() / 100;
            $totalTva += $product->getProductPrice() * $coeff * $product->getProductQuantity(); // ← += et ajoute la quantité
        }
        return $totalTva;
    }
